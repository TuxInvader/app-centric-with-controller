...
The `bucket` is the ID of the bucket which holds your downloads. The `files` should all be placed
in the named `folder` inside the bucket. The `files` section contains the files needed by the 
workshop. Currently the only file(s) store in S3 is the controller installer package.

TIP: The value set here will override values set in the workdir env files during deployment.
