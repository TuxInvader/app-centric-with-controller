...
You will need to generate some traffic. A simple curl script from the gateway would work

----
$ while :; do curl -s https://www.fqdn/ > /dev/null ; curl -s --resolve jenkins.fqdn:443:127.0.0.1  https://jenkins.fqdn/ > /dev/null; sleep 0.1 ; done
----
